...
# from distributed.shuffle._exceptions import CollectiveConsistencyError, CollectiveClosedError
...
CollectiveId = NewType("CollectiveId", str)
...
class RunSpecMessage(OKMessage):
    run_spec: CollectiveRunSpec | ToPickle[CollectiveRunSpec]


class CollectiveRun(Generic[_T_partition_id, _T_partition_type]):
    id: CollectiveId
    run_id: int
    span_id: str | None
    local_address: str
    ...
    def _capture_metrics(self, where: str) -> Iterator[None]:
        """Capture context_meter metrics as
            ...
            {('collective', <span id>, 'foreground|background...', label, unit): value}
        ...
        **Note 1:** When the metric is not logged by a background task
        ...
        This is by design so that one can have a holistic view of the whole collective
        process.

        **Note 2:** We're immediately writing to Worker.digests.
        We don't temporarily store metrics under CollectiveRun as we would lose those
        recorded between the heartbeat and when the CollectiveRun object is deleted at the
        end of a run.
        """
        ...
        def callback(label: Hashable, value: float, unit: str) -> None:
            if not isinstance(label, tuple):
                label = (label,)
            if isinstance(label[0], str) and label[0].startswith("collective-"):
                label = (label[0][len("collective-") :], *label[1:])
            name = ("collective", self.span_id, where, *label, unit)
    ...
    async def barrier(self, run_ids: Sequence[int]) -> int:
        ...
        # TODO: Consider broadcast pinging once when the collective starts to warm
        # up the comm pool on scheduler side
        await self.scheduler.collective_barrier(
    ...
    async def _send(
        ...
        self.raise_if_closed()
        return await self.rpc(address).collective_receive(
    ...
    async def _ensure_output_worker(self, i: _T_partition_id, key: Key) -> None:
        ...
        if assigned_worker != self.local_address:
            result = await self.scheduler.collective_restrict_task(
    ...
    async def _receive(self, data: list[tuple[_T_partition_id, Any]]) -> None:
        """Receive shards belonging to output partitions of this collective run"""
    ...
    def add_partition(
        ...
        # Log metrics both in the "execute" and in the "collective" contexts
        with self._capture_metrics("foreground"):
            with (
                context_meter.meter("collective-shard-partition-noncpu"),
                context_meter.meter("collective-shard-partition-cpu", func=thread_time),
            ):
    ...
    def get_output_partition(
        ...
        with (
            ...
            context_meter.meter("collective-get-output-noncpu"),
            context_meter.meter("collective-get-output-cpu", func=thread_time),
        ):
    ...
    def _get_output_partition(
        ...
        """Get an output partition to the collective run"""
...
def get_worker_plugin() -> CollectiveWorkerPlugin:
    ...
    try:
        ...
        raise RuntimeError(
            "Collectives require Dask's distributed scheduler. This task is not running on a Worker; "
            "please confirm that you've created a distributed Client and are submitting this computation through it."
        ) from e
    try:
        return worker.plugins["collective"]  # type: ignore
    except KeyError as e:
        raise RuntimeError(
            f"The worker {worker.address} does not have a collective plugin."
        ) from e


_BARRIER_PREFIX = "collective-barrier-"


def barrier_key(collective_id: CollectiveId) -> str:
    return _BARRIER_PREFIX + collective_id


def id_from_key(key: Key) -> CollectiveId | None:
    if not isinstance(key, str) or not key.startswith(_BARRIER_PREFIX):
        return None
    return CollectiveId(key[len(_BARRIER_PREFIX) :])
...
class CollectiveRunSpec(Generic[_T_partition_id]):
    ...
    @property
    def id(self) -> CollectiveId:
        return self.spec.id


@dataclass(frozen=True)
class CollectiveSpec(abc.ABC, Generic[_T_partition_id]):
    id: CollectiveId
    disk: bool
    ...
    def create_new_run(
        ...
        return SchedulerCollectiveState(
            run_spec=CollectiveRunSpec(spec=self, worker_for=worker_for, span_id=span_id),
    ...
    def create_run_on_worker(
        ...
        span_id: str | None,
        worker_for: dict[_T_partition_id, str],
        plugin: CollectiveWorkerPlugin,
    ) -> CollectiveRun:
        """Create the new collective run on the worker."""


@dataclass(eq=False)
class SchedulerCollectiveState(Generic[_T_partition_id]):
    run_spec: CollectiveRunSpec
...
@contextlib.contextmanager
def handle_transfer_errors(id: CollectiveId) -> Iterator[None]:
    try:
        yield
    except CollectiveClosedError:
        raise Reschedule()
    except CollectiveConsistencyError:
        raise
    except Exception as e:
        raise RuntimeError(f"Collective {id} failed during transfer phase") from e


@contextlib.contextmanager
def handle_unpack_errors(id: CollectiveId) -> Iterator[None]:
    try:
        yield
        ...
        raise RuntimeError(f"Collective {id} failed during unpack phase") from e
